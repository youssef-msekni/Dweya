...
if(isset($_POST['envoi']))
{
  ...
  if(!(luhn($carte)))
  {
    ...
    $id_comm="\N";
    $requete = "INSERT INTO db_commerce.commandes VALUES('$id_comm', '$id_client','$date')";
  ...
  $tab_id_article=explode("//",$_SESSION["id_article"]);
	...
	$tabdesign=explode("//",$_SESSION["design"]);


    for($i=0;$i<count($tab_id_article);$i++)
    {
      $requete ="INSERT INTO db_commerce.ligne VALUES('$id_comm','".$tab_id_article[$i]."','".$tabquantite[$i]."','".$tabprix_unit[$i]."')";
      $result=$idcom->query($requete);
      //if($result) echo "Ligne $i inserÈe <br />";
    }
  ...
  else echo "Carte bancaire non valable";
}
?>
